...
export function Settings() {
  ...
  const upcomingBirthdays = useMemo(() => {
    return [...birthdays]
      ...
      .sort((a, c2) => a.daysAway - c2.daysAway)
      .slice(0, 4)
  }, [birthdays])
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-5">
        <div className="flex items-center gap-2 mb-3">
          <Cake className="w-5 h-5 text-accent" strokeWidth={1.8} />
          <h3 className="font-display text-lg">Birthdays</h3>
          <button
            onClick={() => nav('/birthdays')}
            className="ml-auto text-[11px] uppercase tracking-wider text-muted hover:text-accent flex items-center gap-1"
          >
        ...
        <label className="flex items-start gap-2 cursor-pointer mb-4">
          <input
            ...
            onChange={(e) => settings.set('birthdayNotifications', e.target.checked)}
            className="mt-1"
          />
          <div>
            <div className="text-sm">Birthday notifications</div>
            <div className="text-xs text-muted leading-relaxed">
              A heads-up the day before and a nudge on the day — so you never get caught off guard.
            </div>
          </div>
        </label>
        {upcomingBirthdays.length === 0 ? (
          <div className="text-xs text-muted/80 leading-relaxed">
            ...
            <button onClick={() => nav('/birthdays')} className="text-accent hover:text-accent2">
              Add the people who matter →
            </button>
          </div>
        ) : (
          <div className={settings.birthdayNotifications ? '' : 'opacity-50'}>
            <div className="text-[10px] uppercase tracking-wide text-muted mb-2">Up next</div>
            <ul className="space-y-1.5">
              {upcomingBirthdays.map(({ b, label }) => (
                <li
                  key={b.id}
                  className="flex items-center gap-3 rounded-lg border border-border bg-surface2/30 px-3 py-2"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-[11px] text-muted">
                      {format(new Date(2000, b.month - 1, Math.min(b.day, daysInMonth(2000, b.month))), 'MMMM d')}{' '}
